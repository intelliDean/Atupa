...
fn get_invoke_regex() -> &'static Regex {
    INVOKE_REGEX.get_or_init(|| Regex::new(r"Program (?P<addr>[1-9A-HJ-NP-Za-km-z]{32,44}) invoke \[(?P<depth>\d+)\]").unwrap())
}
...
fn get_return_regex() -> &'static Regex {
    RETURN_REGEX.get_or_init(|| Regex::new(r"Program (?P<addr>[1-9A-HJ-NP-Za-km-z]{32,44}) (?P<status>success|failed)").unwrap())
}
